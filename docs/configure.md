...
**Prefix suggestion and live preview.** Both rule-creation paths — the Rules-screen New-rule form and the incident-drawer Ignore form — call `POST /ignore-rules/preview` as you type. The endpoint returns `open_count`, `past_count`, `total_count`, a `suggested_alarm_name_prefix`, and up to 10 sample incidents; the form renders a live line such as `Matches 3 open · 12 past (15 total)` in amber when any open incidents are affected, and an explicit consequence line `Creating this rule will resolve N open incident(s).` The suggested prefix is the longest separator-aligned prefix of the target alarm name that still groups it with at least one other alarm Relay has already seen — alignment to a separator boundary (`-`, `_`, `/`, `.`, `:`) is intentional so the prefix does not cut mid-token. The `alarm_name_prefix` field performs a plain `startswith` match, not a glob or regex. The form displays it with a trailing `*` for readability (e.g. `ABCDE-ABCDE-*`), but the value stored in DynamoDB and in `routing.yaml` is the bare prefix `ABCDE-ABCDE-`. The preview endpoint is read-only (no writer auth required) and shares the same `IgnoreRule.matches()` code path as enforcement, so the displayed count can never drift from what the rule actually does.
...
> The Ignore action in the incident drawer pre-fills a rule from the open incident using `alarm_name_prefix` scope by default — the scope toggle order is `Alarm name prefix (recommended)`, `This exact alarm`, `All from app`. The prefix field is pre-filled with the server-suggested maximal prefix: the longest separator-aligned prefix of the triggering alarm that groups it with at least one other alarm Relay has already seen. A live count shows how many open and past incidents match before you save. Creating the rule auto-resolves **every** currently open matching incident (not just the one in the drawer) — each with an "ignored" timeline event and a RESOLVED dispatch (closing any linked external ticket).
