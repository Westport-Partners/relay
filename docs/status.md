...
| Tag-aware incident drawer (resource tags + resolved metadata) | ✅ | `hub/dashboard_parts/` (`renderIncident` "Resolved metadata" + "Resource tags" sections; shared `metaValueHtml` helper); `hub/app.py` (`GET /incidents/{id}` returns full incident) | The incident drawer renders `deployment_metadata` (as a kv grid; `pipeline_url`→link, `git_sha`→short+title) then `incident.tags` (as tag chips). The same `metaValueHtml` helper renders `pipeline_url`/`git_sha` consistently in the tile drawer's Metadata section. |
...
| GitLab token UI setting (settings store overrides Secrets Manager) | ✅ | `hub/app.py` (`PUT /settings/gitlab-token`, `/settings/gitlab-token/test`, token provider); `hub/dashboard_parts/` (Settings card) | Runtime-set token, masked on read; overrides the Secrets Manager fallback live. The Test button verifies the full create-issue capability — auth (`GET /user`), `api` scope (`GET /personal_access_tokens/self`), and Reporter+ access on an optional target project (`GET /projects/:id`). Info bubble documents required `api` scope + Reporter role. |
| ServiceNow credentials UI setting (settings store overrides env fallback) | ✅ | `hub/app.py` (`PUT /settings/servicenow-credentials`, `/settings/servicenow-credentials/test`, credential providers); `adapters/integrations/servicenow/sink.py` (provider precedence in `_instance_url`/`_username`/`_password`, `test_connection`); `hub/dashboard_parts/` (Settings card) | At parity with the GitLab token: instance URL + username + password set on the Settings screen, stored in DynamoDB, masked on read, resolved live per request (overrides the `RELAY_SERVICENOW_*` env/Secrets-Manager fallback). The Test button validates against `GET /api/now/table/incident`. |
