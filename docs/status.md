...
| Global environment filter (header lens: ALL/prod/test/dev) | ✅ | `hub/dashboard_modules/env-filter.js` (`matchesEnv`, persistence), `state.js` (`activeEnv`), `fleet.js`/`incidents.js`/`metrics.js` consumers, `metrics-compute.js` (client KPI port), `02-body-shell.part.html` `#env-filter`; `hub/app.py` (`/incidents` + `/incidents/history` serialization enrichment) | ✨ Net-new. A sticky header control scopes Fleet, Incidents, and Metrics to one environment; persists in `localStorage` (key `relay.env-filter`) across nav + reload. Environment is the namespace **above** the org hierarchy — composes with the Fleet incidents-only filter and grouping. Client-side only (no schema): under a specific env, Metrics recomputes KPIs in `metrics-compute.js`, a parity-tested mirror of `core/metrics.py` (`tests/test_metrics_parity.py`); under ALL it uses `GET /metrics`. [#40](https://github.com/Westport-Partners/relay/issues/40). |
| Fleet big-board across all apps | ✅ | `hub/fleet_store.py`; dashboard `fleet.js` + `fleet-groups.js` | ✨ Net-new vs AWS IM. Tiles **grouped by org hierarchy** with rollup status/count headers (prefers `GET /fleet/rollup`, client fallback), org-less under "Ungrouped"; tiles wrap full name + show open/severity, on-call-gap, env, owner indicators; per-group space-adaptive density (`auto-fit` + count-aware `--tile-min`, reflows on resize). [#37](https://github.com/Westport-Partners/relay/issues/37). Browser-verified 2026-06-27 (4 groups, narrow-width reflow, filter, tile→drawer). |
