...
| UI-managed ignore rules (drop matching alarms entirely) | ✅ | `config/schema.py` (`IgnoreRule`, `IgnoreConfig`); `adapters/aws/dynamo_stores.py` (`DynamoIgnoreRuleStore`); `node/handler.py` (`_matched_ignore_rule`); `hub/app.py` (`/ignore-rules` routes, `_seed_ignore_rules`, `POST /incidents/{id}/ignore`, `POST /ignore-rules/preview`); `core/matching.py` (`suggest_alarm_prefix`, `separator_aligned_prefixes`); `hub/dashboard_modules/rules.js` + `rule-forms.js` (Rules view, Ignore action in incident drawer); `tests/core/test_matching.py` (49 tests); `tests/hub/test_hub_ignore_rules.py` (sibling backfill, preview endpoint, writer-gating) | Distinct from `suppression:` (which rate-limits but eventually pages) — an ignored alarm is dropped at the Node before persist, page, ticket, or federation, and is automatically excluded from all metric rollups. `routing.yaml`'s optional `ignore:` block seeds the DynamoDB store on first boot; thereafter DynamoDB is the runtime source of truth and the UI edits it directly ("DB wins"). The Rules screen shows trigger counts, reason/note, created_by/at, and a deviation banner + YAML download when live rules differ from the routing.yaml baseline. Old `/rules` paths are deprecated aliases for `/ignore-rules`. **Both** create paths (`POST /ignore-rules` and `POST /incidents/{id}/ignore`) share one backfill: creating a rule resolves every matching open incident with an `"ignored"` timeline event, dispatches RESOLVED (closes external tickets), increments the rule's trigger count, and recomputes the affected fleet tiles; the originating incident is never double-stamped. `POST /incidents/{id}/ignore` returns `matched_incident_count`. The scope defaults to `alarm_name_prefix` (recommended) in both forms; the order is `Alarm name prefix (recommended)` → `This exact alarm` → `All from app`. `POST /ignore-rules/preview` (no writer auth required) powers live match counts and prefix suggestion in both forms: returns `open_count`, `past_count`, `total_count`, `suggested_alarm_name_prefix` (longest separator-aligned prefix of `target_alarm_name` that groups it with ≥1 sibling; `null` when none qualifies), and up to 10 `samples`; `no_matcher=true` when the body carries no matcher field (HTTP 200, zero counts). Preview and enforcement share the same `IgnoreRule.matches()` path — counts cannot drift. `core/matching.py` is AWS-free: `SEPARATORS`, `longest_common_prefix`, `trim_to_separator`, `separator_aligned_prefixes`, `suggest_alarm_prefix`; 49-test suite in `tests/core/test_matching.py`. |
...
| Paging subscription state + Subscribe button on Contacts | ✅ | `hub/app.py` (`GET /contacts/subscriptions`, `POST /contacts/{id}/subscribe`); `adapters/aws/sns_notifier.py` (`list_subscription_status_by_email`, `subscribe_email`); `hub/dashboard_modules/contacts.js` (lazy hydration); `infra/stacks/compute_stack.py` (`sns:ListSubscriptionsByTopic`/`sns:Subscribe` on the team paging topic) | #78. The Contacts "Paging" column resolves per-contact **email** subscription state against the paging topic and lazily hydrates each row after render (never blocks page load): unsubscribed → **Subscribe** (sends SNS confirmation, shows "Pending confirm"); confirmed / no-email / topic-unavailable → existing **Test page**. One `ListSubscriptionsByTopic` serves all rows, cached 30s. "Subscribed" tracks the topic's email subscriptions only; direct-to-phone SMS is the separate opt-in tracked under Engagement (#83). Last verified against code: 2026-07-06. |
...
| SMS delivery (topic path) | 🟡 | `adapters/aws/sns_notifier.py` (`publish_direct`); `infra/stacks/compute_stack.py` (`RelayHubDirectSms`, emitted non-BYOR + BYOR, gated `relay:enable_direct_sms`) | Code exists; **direct-to-phone is gated**: needs `relay:enable_direct_sms` IAM grant + AWS SNS sandbox exit. The grant is scoped by `aws:RequestedRegion` (the earlier `sns:Protocol` condition was Subscribe-only and failed closed on `Publish`). See [`integrations.md`](integrations.md). Last verified 2026-07-06. |
...
| Incidents list: Open vs History tabs (state-correct) | ✅ | `hub/app.py` (`GET /incidents` open-only; `GET /incidents/history` filters to `{RESOLVED, CLOSED}`); `hub/dashboard_modules/incidents.js` + `incident-store.js` | The History tab shows only terminal-state incidents; open (`TRIGGERED`/`ACKNOWLEDGED`/`ESCALATED`) never leak in. Both tabs are filtered views of one client-side incident store, kept in sync with the board via SSE. Last verified against code: 2026-06-27. |
| Incident listing scales by GSI Query (not full-table Scan) | ✅ | `adapters/aws/dynamo_stores.py` (`DynamoIncidentStore.list_open_incidents`/`list_incidents` → `_query_incident_index`; `_to_item` derives `gsi_open_pk`/`gsi_all_pk` from state); `infra/stacks/data_stack.py` (`incident-status-index` sparse-open + `incident-all-index`) | `list_open_incidents` queries a **sparse** GSI (`gsi_open_pk="OPEN"`, only open incidents carry it) and `list_incidents` queries a constant-partition all-index, both sorted by `created_at` newest-first and paginated via `LastEvaluatedKey`. Reads are proportional to matching incidents, not total table size — scales to ~200 apps on the federated hub table. GSI keys are pure functions of `incident.state` computed in `_to_item`, so a resolve overwrite evicts the row from the open index with no explicit `REMOVE`. Replaced the orphaned `status`/`opened_at` index that incident rows never populated. Resolves issue #23. Last verified against code: 2026-06-28. |
...
| Temporal purge of incidents + metrics | ✅ | `adapters/aws/dynamo_stores.py` (`DynamoIncidentStore.purge_incidents` returns `affected_tiles`); `hub/app.py` (`POST /admin/purge` recomputes each affected tile + pushes SSE delta, `tiles_recomputed`); dashboard Maintenance view | Before/after timestamp bound or synthetic-only; dry-run preview; cascades to companion `ESC#` rows. Writer-gated; refuses an unbounded non-dry-run purge. Purge recomputes affected fleet tiles and pushes a delta so the big board clears live — no heartbeat or restart needed. Last verified against code: 2026-06-27. |
...
| Global environment filter (header lens, **data-derived** ALL + present envs) | ✅ | `hub/dashboard_modules/env-filter.js` (`matchesEnv`, persistence, `buildEnvFilter` dynamic buttons), `fleet-groups.js` (`envsPresent`), `state.js` (`activeEnv`), `fleet.js`/`incidents.js`/`metrics.js` consumers, `metrics-compute.js` (client KPI port), `stream.js`/`main.js` (rebuild on data arrival), `02-body-shell.part.html` `#env-filter`; `hub/app.py` (`/incidents` + `/incidents/history` serialization enrichment) | ✨ Net-new. A sticky header control scopes Fleet, Incidents, and Metrics to one environment; persists in `localStorage` (key `relay.env-filter`) across nav + reload, **reconciling to ALL when a persisted env is no longer present**. Choices are **derived from the environments actually in the fleet** (canonical promotion order; includes non-standard envs like `pre-prod`) — no longer a hardcoded set. Environment is the namespace **above** the org hierarchy — composes with the Fleet incidents-only filter and grouping. Client-side only (no schema): under a specific env, Metrics recomputes KPIs in `metrics-compute.js`, a parity-tested mirror of `core/metrics.py` (`tests/test_metrics_parity.py`); under ALL it uses `GET /metrics`. [#40](https://github.com/Westport-Partners/relay/issues/40), dynamic set [#43](https://github.com/Westport-Partners/relay/issues/43). |
| Fleet big-board across all apps | ✅ | `hub/fleet_store.py`; dashboard `fleet.js` + `fleet-groups.js` | ✨ Net-new vs AWS IM. **Environment-container board**: outer block per environment present (derived dynamically; AWS account is never an axis), with the org hierarchy **nesting to its actual depth** inside each block and **every level packing children horizontally** (`auto-fit` grids — never `auto-fill`). A run of single-child tile-less levels **collapses to one breadcrumb header**; multi-child levels stay expanded. **One render path** for Team (shallow → collapse) and Central (deep tree). Node status/counts computed over the visible in-env tiles; worst-first ordering; org-less tiles under "Ungrouped" within their env. Leaf tile grid is space-adaptive (count-aware `--tile-min`, reflows on resize). [#43](https://github.com/Westport-Partners/relay/issues/43) (replaces component-level grouping [#37](https://github.com/Westport-Partners/relay/issues/37)/[#39](https://github.com/Westport-Partners/relay/issues/39)). Browser-verified 2026-06-28 (3 env blocks dev/test/prod, side-by-side packing, single-child collapse breadcrumb, env select → one block, dynamic env buttons, incidents filter, tile→drawer). |
| Tile open-count is derived + self-healing | ✅ | `hub/fleet_store.py` (`recompute`); `hub/app.py` (`HubState.recompute_tile`, `_recompute_incident_tile` on resolve/ack/ignore, `SweepTimer._do_sweep` reconciliation); `adapters/aws/cloudwatch_source.py` (`account_id` from event envelope) | The per-event delta only runs on the ingest path; every UI/harness write + the 30s sweep recompute the count from `list_open_incidents`, so a missed decrement self-heals instead of leaving the board red. Incident `account_id` binds to the event's `account` so it lands on the heartbeat-registered tile key (no orphan count). |
| Build provenance (git SHA) on Settings | ✅ | `hub/app.py` (`GET /config` → `build.git_sha`/`built_at`); `hub/dashboard_modules/settings.js`; `Dockerfile` (`RELAY_BUILD_SHA`/`_TIME` ARG→ENV); `docker-compose.yml` (build args); `.github/workflows/publish-image.yml` | The running container reports the commit it was built from on the Settings screen, so "is my branch deployed?" is a glance. |
| Tile detail drawer (click a tile → deployment detail) | ✅ | `hub/dashboard_modules/tile-drawer.js` (`openTile`/`renderTile`); `hub/app.py` (`GET /fleet/tile?account_id=&app_name=` fills `on_call`); `hub/health.py` (`FleetTile.org_path`/`metadata`/`on_call`) | One data-driven drawer for BOTH topologies — sections (on-call, hierarchy, metadata, AWS tags, open incidents) render only when data is present. Team Hub fills on-call live; federated Hub shows the owning team's pushed `on_call` snapshot. Tile lookup uses query params (not path segments) so an `app_name` containing `/` (e.g. an ECS autoscaling alarm's `TargetTracking-service/...`) resolves instead of 404ing. Last verified 2026-06-28. |
...
| BYOR — import pre-provisioned roles, emit inline-policy JSON | ✅ | `compute_stack.py` (`relay:ecs_{task,execution}_role_arn`; `_emit_byor_outputs`; new IAM gated on `byor_mode`); `tests/infra/test_compute_byor.py` (`test_byor_task_policy_is_literal_json_no_intrinsics`) | Net runtime IAM surface is one task role + one exec role. `_emit_byor_outputs` builds Resource ARNs as **literal strings** (partition/account parsed from the supplied task-role ARN, region from context) so `ByorTaskRoleInlinePolicy` is plain pasteable JSON — it previously embedded `Fn::ImportValue`/`Fn::GetAtt`/`Ref` intrinsics from CDK tokens and could not be pasted into the IAM console. Last verified 2026-07-07 (live BYOR→ECS deploy). |
...
| Deploy via CloudFormation, no bootstrap (locked-down accounts) | ✅ | `scripts/relay-deploy-direct.sh` (cdk synth → `aws cloudformation deploy` with caller creds; CLI-orphan guard now skips resources already owned by a Relay stack via `_relay_stack_exists`; opt-in `RELAY_CFN_MODE=EXPRESS` fast path, rollback retained); `infra/app.py` (`DefaultStackSynthesizer(generate_bootstrap_version_rule=False)` — no `/cdk-bootstrap` SSM lookup → no `ssm:GetParameters` needed); `scripts/relay-preflight.sh` (`aws-cli-express` WARN); `prompts/deploy-byor.md`, `docs/byor.md`, `docs/deploy.md` | No CDK assets + suppressed bootstrap-version rule → synthesized templates deploy with zero bootstrap dependency. The CLI-orphan guard no longer false-positives on the documented data→compute sequence (it only flags resources with no owning stack). **ECS still requires scoped `iam:PassRole`** on the deploy identity (task+exec roles → `ecs-tasks`); docs now spell out the deploy-identity policy. `RELAY_CFN_MODE=EXPRESS` needs AWS CLI ≥ 2.35. Last verified 2026-07-07 (live BYOR→ECS deploy on a permissions-boundary-constrained account). |
...
| Native Terraform/Terragrunt IaC path (CDK alternative) | ✅ | `infra/terraform/modules/{data-plane,compute,federation}`; `infra/terraform/live/` (Terragrunt per-env); `tests/infra/test_terraform_parity.py`; `docs/deploy.md` (Terraform path) | Hand-written TF at parity with the CDK stacks + CLI provisioner; CDK stays canonical, parity test guards drift. Compute module is **BYOV + BYOR required** (never creates a VPC or IAM role) and emits the inline-policy/trust JSON as outputs. Terragrunt wires `data-plane → compute` per environment. (#52) |
| Deep readiness probe (`GET /health/ready`) | ✅ | `hub/app.py` (`health_ready`, `_ready_error`); `tests/hub/test_health_ready.py` | Seven checks (DynamoDB, SQS ingest, SNS paging topic, SNS direct-SMS IAM, config loaded, ignore-rule count, routing-rule count). All probes are cheap/bounded (describe-only — no publishes/sends). The SNS direct-SMS probe only runs when `RELAY_ENABLE_DIRECT_SMS=true` (else skipped with a note). It calls `sns:ListPhoneNumbersOptedOut`, which stays within SNS; it deliberately avoids `sns:CheckIfPhoneNumberIsOptedOut`, which AWS routes to Pinpoint SMS Voice (`sms-voice:DescribeOptedOutNumbers`) and which produced a false `degraded` in strict-SCP accounts even when direct SMS worked at runtime (ISSUE-5). The BYOR inline policy (CDK + Terraform) grants the three probe actions (`dynamodb:DescribeTable`, `sns:GetTopicAttributes`, `sns:ListPhoneNumbersOptedOut`). `status: "degraded"` names the failing check and AWS error code. Documented in `docs/byor.md` and `prompts/troubleshoot-deploy.md` as the first diagnostic step. (#80) |
