...
| Deep readiness probe (`GET /health/ready`) | ✅ | `hub/app.py` (`health_ready`, `_ready_error`); `tests/hub/test_health_ready.py` | Seven checks (DynamoDB, SQS ingest, SNS paging topic, SNS direct-SMS IAM, config loaded, ignore-rule count, routing-rule count). All probes are cheap/bounded (describe-only — no publishes/sends). The SNS direct-SMS probe only runs when `RELAY_ENABLE_DIRECT_SMS=true` (else skipped with a note), so a strict-SCP account that never opted into direct SMS is not falsely `degraded` by a Pinpoint SMS Voice deny. The BYOR inline policy (CDK + Terraform) grants the three probe actions (`dynamodb:DescribeTable`, `sns:GetTopicAttributes`, `sns:CheckIfPhoneNumberIsOptedOut`). `status: "degraded"` names the failing check and AWS error code. Documented in `docs/byor.md` and `prompts/troubleshoot-deploy.md` as the first diagnostic step. (#80) |
