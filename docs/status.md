...
| Deploy without `iam:PassRole` (locked-down accounts) | ✅ | `scripts/relay-deploy-direct.sh` (cdk synth → `aws cloudformation deploy` with caller creds; opt-in `RELAY_CFN_MODE=EXPRESS` fast path via raw `create/update-stack --deployment-config`, rollback retained); `scripts/relay-preflight.sh` (`aws-cli-express` WARN); `prompts/deploy-byor.md`, `prompts/upgrade-aws-cli.md`; `docs/deploy.md` (Locked-down accounts section) | No CDK assets, so synthesized templates deploy directly. Data plane creates zero roles/VPC → deploys in the most restricted accounts. `RELAY_CFN_MODE=EXPRESS` (default `STANDARD`) returns once resource config is applied for faster iterative deploys; needs AWS CLI ≥ 2.35. Last verified 2026-07-06. |
